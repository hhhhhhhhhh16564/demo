...
-(void)mohuEffect{
    ...
    GPUImagePicture *picProcess = [[GPUImagePicture alloc]initWithImage:sourceImage];
    
    // 2.2 添加需要处理的滤镜
    GPUImageGaussianBlurFilter *bulurFilter = [[GPUImageGaussianBlurFilter alloc]init];
    
    // 设置纹理和模糊程度
    //纹理
    bulurFilter.texelSpacingMultiplier = 3;
    
    //模糊
    bulurFilter.blurRadiusInPixels = 2;
    
    [picProcess addTarget:bulurFilter];
    
    
    //2.3 处理图片
    [bulurFilter useNextFrameForImageCapture];
    
    [picProcess processImage];
    
    
    // 2.4取出新的图片
    UIImage *image = [bulurFilter imageFromCurrentFramebuffer];
...
//    GPUImageStillCamera->GPUImageFilter->GPUImageView 这样,摄像机转到滤镜再转到view上显示出来.
    ...
    GPUImageView *showViwe = [[GPUImageView alloc]initWithFrame:self.view.bounds];
    [self.view insertSubview:showViwe atIndex:0];
    
    [filter addTarget:showViwe];
...
-(void)realTimeCamera{
    ...
    self.stillCamera.delegate = self;
